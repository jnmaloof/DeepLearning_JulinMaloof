...
val_generator <- function() {
  list(input = generator_next(val_img_generator),
    target = generator_next(val_target_generator))
}

# make sure that the range of the mask is correct

train_generator()[[2]] %>% as.vector() %>% unique()
```
...
callbacks <- list(
  ...
  callback_model_checkpoint( filepath="CUB_segmentation_model.h5",
                               monitor="val_loss", 
                                    save_best_only=TRUE)
)
...
### set up directories
```{r}
test_dir <- "input/CUB_200_2011/test/test_bird_images/"

test.df = tibble(image=dir(test_dir, pattern="\\.jpg$", full.names = TRUE, recursive = TRUE))
...
```{r}
train_img_generator <- flow_images_from_dataframe(dataframe = test.df, 
                                            x_col="image",  
                                            generator = image_data_generator(rescale=1/255),
                                            #target_size = c(200,200),
                                            class_mode = NULL,
                                            batch_size = nrow(test.df),
                                            shuffle = FALSE)
```

### load model and predict
```{r}
model <- load_model_hdf5("input/CUB_200_2011/test/CUB_segmentation_model_20.hdf5")
```
