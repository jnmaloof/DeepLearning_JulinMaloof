...
info.sm <- info %>%
  ...
  mutate(group=cut(percent_rank(row_number()),
         ...
         file.path=file.path(getwd(), "open_plants", "images", eppo, filename)) 
head(info.sm)
```
...
train_gen <- flow_images_from_dataframe(
  ...
  y_col=list("eppo", "growth_condition") # Can't be named list, apparently
)
